...
# ═════════════════════════════════════════════════════════════════════════════
# STEP 2: IMAGES — Pollinations default → flux → placeholder
# ═════════════════════════════════════════════════════════════════════════════
def _pollinations(prompt: str, scene_no: int, model: str = "") -> str:
...
r.raise_for_status()
# Check it's actually an image (not error page)
if len(r.content) < 5000:
raise ValueError(f"Response too small ({len(r.content)} bytes) - likely error")
...
try:
path = _pollinations(prompt, n, **kwargs)
print(f" ✅ Pollinations {label}")
break
except Exception as e:
print(f" ❌ Pollinations {label}: {e}")
time.sleep(3)
if not path:
path = _placeholder(n)
